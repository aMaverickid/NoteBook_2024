...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i // 2 + (len(deck) // 2) * (i % 2)] for i in range(0, len(deck))]
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for br_t in branches(t):
            if berry_finder(br_t):
                return True
        return False
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
      return tree(label(t), [tree(i) for i in leaves])
        ...
        return tree(label(t), [sprout_leaves(sub_tree, leaves) for sub_tree in branches(t)])
...
def add_trees(t1, t2):
    ...
    #     res_branches.append(add_trees(branches(t1)[i], branches(t2)[i]))
    #     i = i + 1 
   ...
   ### Version 2 zip
    ...
    for b1, b2 in zip(branches(t1), branches(t2)):
        res_branches += list(add_trees(b1, b2))
    i = len(res_branches)
    res_branches += branches(t1)[i:]
    res_branches += branches(t2)[i:]
    return tree(res_label, res_branches)
